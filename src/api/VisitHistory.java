...
public class VisitHistory extends HttpServlet {
	 ...
	 */
    protected void doGet(HttpServletRequest request, HttpServletResponse response) //读到所有的history
			throws ServletException, IOException {
		try {
 //          DBConnection connection = new MySQLDBConnection();
			JSONArray array = null;
			if (request.getParameterMap().containsKey("user_id")) { //和dopost不一样，都是把request里面的参数拿出来
																	//因为get的信息是在url里的，但是post信息在body里所不用额外工作
				String userId = request.getParameter("user_id");
				Set<String> visited_business_id = connection.getVisitedRestaurants(userId);
				array = new JSONArray();
				for (String id : visited_business_id) {
					array.put(connection.getRestaurantsById(id, true)); //true说明 爱心是实心的
				}
