 ...
 */
function init() {
  // Register event listeners 注册了各种按钮 
...
function loadNearbyRestaurants() {
  ...
  activeBtn('nearby-btn');  /* 去active 那个按钮 */	

  // The request parameters
  var url = './restaurants';
  var params = 'user_id=' + user_id + '&lat=' + lat + '&lon=' + lng;
  var req = JSON.stringify({}); /* 和JSON.parse(res)是一对*/	
  ...
  ajax('GET', url + '?' + params, req, 
    // successful callback
    function (res) {
      var restaurants = JSON.parse(res);  /* 得到一个数组*/	
...
function addRestaurant(restaurantList, restaurant) {
  ...
  // set the data attribute 想办法把数据保存在html里 下面是通过data去修改html
